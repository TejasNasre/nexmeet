...
const Card: React.FC<CardProps> = ({ title, description, image, bgColor, isFullGif }) => {
  ...
  return (
    ...
    <motion.div
      ref={cardRef}
      className={`w-full max-w-sm mx-auto h-[400px] rounded-3xl overflow-hidden ${isFullGif ? '' : 'p-8 flex flex-col justify-between'} ${bgColor}`}
      animate={controls}
    ...
    >
      {isFullGif ? (
        <div className="relative w-full h-full">
          <Image src={image} alt={title} layout="fill" objectFit="cover"  className='w-full h-full' unoptimized/>
          <div className="absolute inset-0 flex flex-col justify-between">
            <div className='p-4'>
              <h3 className="text-2xl font-bold text-white mb-2 text-shadow">{title}</h3>
...
const FeatureCards: React.FC = () => {
  ...
  const cards: CardProps[] = [
    {
      ...
      description: "Explore event spaces in your area and book them for your next event",
      image: "https://media.giphy.com/media/bNqswZuM0Zf0yLHp5b/giphy.gif?cid=790b7611wfz0vclzoce7q2x93roruh4xsa4fqpabvzkp0io6&ep=v1_gifs_search&rid=giphy.gif&ct=g",
    ...
    },
    {
      ...
      description: "We respect everybody's choices and try to make it as per your needs",
      image: "https://i.giphy.com/media/v1.Y2lkPTc5MGI3NjExcXQ1bDA5ZXhuczBhdmM0YWJjcDF3aDZqcHQ3M2lib2tyMDh0bWt3ayZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/REPL2BIiGhyFO/giphy.gif",
      bgColor: "bg-gradient-to-b from-purple-400 to-pink-600",
      isFullGif: true
    },
    {
      title: "Network to get work",
      description: "Connect with like-minded people out there and grab those opportunities.",
    ...
    },
    {
      ...
      description: "Manage registrations and ticket sales effortlessly for your events.",
    ...
    },
    {
      ...
      description: "Organiser can invite anyone and have that ability to reject or approve",
  ...
  const nextCard = () => setCurrentIndex((prevIndex) => 
    Math.min(prevIndex + 1, cards.length - visibleCards)
  );
  const prevCard = () => setCurrentIndex((prevIndex) => Math.max(prevIndex - 1, 0));
          ...
          <div className="flex space-x-2">
            <button
              ...
              className={`rounded-full p-2 transition-colors duration-300 ${
                isFirstCard ? 'bg-gray-600 text-gray-400' : 'bg-indigo-600 text-white'
              }`}
            ...
            <button
              onClick={nextCard}
              className={`rounded-full p-2 transition-colors duration-300 ${
                isLastCard ? 'bg-gray-600 text-gray-400' : 'bg-indigo-600 text-white'
              }`}
