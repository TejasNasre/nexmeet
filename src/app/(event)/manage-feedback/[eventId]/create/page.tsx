...
import { TextInputField, RatingField, SliderField, MCQField, RadioField } from "@/components/form-fields/form-fields";
...
export default function CreateFeedbackForm() {
    ...
    const handleAddField = () => {
        const newField: FormField = {
            ...
            label: "",
            value: "",
            options: currentFieldType === 'mcq' || currentFieldType === 'radio' 
                ? ["Option 1", "Option 2"] 
                : undefined
        };
    ...
    const handleAddOption = (fieldIndex: number, newOption: string) => {
        setFormFields(formFields.map((field, idx) => {
            if (idx === fieldIndex) {
    ...
    const handleDeleteOption = (fieldIndex: number, optionIndex: number) => {
        setFormFields(formFields.map((field, idx) => {
            if (idx === fieldIndex) {
                const updatedOptions = field.options?.filter((_, optIdx) => optIdx !== optionIndex) || [];
                return { ...field, options: updatedOptions };
            }
            return field;
        }));
    ...
    const handleOptionChange = (fieldIndex: number, optionIndex: number, newValue: string) => {
        setFormFields(formFields.map((field, idx) => {
            if (idx === fieldIndex) {
            ...
            case "mcq":
                return (
                    <MCQField
                        ...
                        onOptionChange={(optionIdx, value) => handleOptionChange(index, optionIdx, value)}
                    />
                );
            case "radio":
                return (
    ...
    return isAuthenticated ? (
        <div className="w-full h-auto bg-black text-white py-[8rem] px-[2rem] flex flex-col justify-center items-center">
            <form onSubmit={handleSubmit} className="w-full flex flex-col flex-wrap gap-10 md:w-2/3">
